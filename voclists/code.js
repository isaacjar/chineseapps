...
function startGame() {
	if (!vocabulary || vocabulary.length === 0) {
		showToast("No vocabulary loaded");
		return;
	}
  currentQuestion = 0;
  correctAnswers = 0;
...
function cargarListado(nombre) {
  fetch(`https://isaacjar.github.io/chineseapps/voclists/${nombre}.json`)
    ...
    .then(data => {
      vocabulary = data;
      nombreListadoActual = nombre;
	  miscel = misc; 
      document.getElementById('questionLabel').textContent = `Loaded: ${misc}`;
	  document.getElementById('title').textContent = `Vocabulary Game – ${nombre}`;
...
function mostrarPopup(listados) {
  ...
  titulo.textContent = "Selecciona un listado de vocabulario";
