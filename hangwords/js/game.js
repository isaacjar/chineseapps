...
function updateDisplay(){
    ...
    if(!wordArea) return;
    wordArea.textContent = currentWordDisplay.map(c=>c===" "?" ":c).join(" ");
}
...
async function startGame(){
    if(!window.currentVoc || Object.keys(window.currentVoc).length===0){
        toast("No vocabulary loaded!"); return;
    }
    mistakes=0;
...
function nextWord(){
    ...
    // Usar lista personalizada si existe
    let vocArray;
    if(window.useCustomWords && Array.isArray(window.customWordList)){
        vocArray = window.customWordList.map(w=>({pin:w}));
    } else {
        vocArray = Object.values(window.currentVoc);
    }
